...
<div class="posts-page">
    <div class="feed-container">
        <h1 class="page-title"><?php echo e(__('app.nav.posts')); ?></h1>

        <!-- Composer Card -->
        <div class="composer-card">
            <div class="composer-inner">
                <?php if(auth()->guard()->check()): ?>
                <?php $me = auth()->user(); $meAvatar = get_avatar_url($me?->avatar); ?>
                <a href="<?php echo e(route('profile.show')); ?>" title="<?php echo e(__('app.nav.profile')); ?>">
                    <div class="composer-avatar" style="background-image: url('<?php echo e($meAvatar); ?>')"></div>
                </a>
                <?php else: ?>
                ...
                <button class="composer-trigger" onclick="openComposerModal()">
                    <?php echo e(__('app.feed.whats_on_your_mind')); ?>
        ...
        <div class="empty-state">
            ...
            <div class="empty-actions">
                <a href="<?php echo e(route('search.view')); ?>" class="btn-primary-glow">
                    <i class="fas fa-user-plus"></i> Tìm bạn bè
                </a>
                <a href="<?php echo e(route('search.view')); ?>" class="btn-secondary-outline">
        ...
        <div class="post-card" data-post-card="<?php echo e($post->id); ?>">
            <div class="post-body">
                <!-- Header -->
                <div class="post-header">
                    <div class="post-author">
                        <a href="<?php echo e((auth()->id() === $post->user_id) ? route('profile.show') : route('profile.show-user', $post->user_id)); ?>">
                            <?php $avatarUrl = get_avatar_url($post->user?->avatar); ?>
                            <div class="post-avatar" style="background-image: url('<?php echo e($avatarUrl); ?>')"></div>
                        </a>
                        <div class="post-author-info">
                            <a href="<?php echo e((auth()->id() === $post->user_id) ? route('profile.show') : route('profile.show-user', $post->user_id)); ?>" class="post-author-name">
                                <?php echo e($post->user->name ?? 'User'); ?>

                            </a>
                            <span class="post-time" data-timestamp="<?php echo e($post->created_at->toIso8601String()); ?>">
                    ...
                    <?php if(auth()->guard()->check()): ?>
                    <?php $canDelete = (auth()->id() === $post->user_id) || (in_array(auth()->user()->user_role,['admin','super_admin'])); ?>
                    <?php if($canDelete): ?>
                    <div style="position: relative;">
                        <button class="post-menu-btn" onclick="toggleDropdown('postMenu<?php echo e($post->id); ?>')">
                            <i class="fas fa-ellipsis-h"></i>
                        </button>
                        <div class="post-dropdown" id="postMenu<?php echo e($post->id); ?>">
                            <button class="post-dropdown-item" onclick="openEditModal(<?php echo e($post->id); ?>)">
                                <i class="fas fa-pen"></i> Chỉnh sửa
                            </button>
                            <div class="post-dropdown-divider"></div>
                            <form method="POST" action="<?php echo e(route('posts.destroy', $post)); ?>" class="delete-post-form" data-post-id="<?php echo e($post->id); ?>">
                ...
                <!-- Shared Post -->
                <?php if($post->shared_post_id && $post->sharedPost): ?>
                <?php $sp = $post->sharedPost; $spAvatar = get_avatar_url($sp->user?->avatar); ?>
                <div class="shared-post">
                    <?php if($sp->media && $sp->media->count()): ?>
                        <?php $spImages = $sp->media->where('type','image'); ?>
                        <?php if($spImages->count() == 1): ?>
                            <img src="<?php echo e(asset('storage/'.$spImages->first()->path)); ?>" class="post-media-single">
                        <?php elseif($spImages->count() >= 2): ?>
                            <div class="post-media-grid cols-2">
                                <?php $__currentLoopData = $spImages->take(4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $im): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="media-item">
                                    <img src="<?php echo e(asset('storage/'.$im->path)); ?>">
                                    <?php if($i == 3 && $spImages->count() > 4): ?>
                                    <div class="media-more-overlay">+<?php echo e($spImages->count() - 4); ?></div>
                                    <?php endif; ?>
                                </div>
                    ...
                    <div class="shared-author">
                        <img src="<?php echo e($spAvatar); ?>" class="shared-avatar">
                        <div>
                            <div class="shared-name"><?php echo e($sp->user->name ?? 'User'); ?></div>
                            <div class="shared-time"><?php echo e(optional($sp->created_at)->diffForHumans()); ?></div>
                        </div>
                    </div>
                    <?php if($sp->content): ?>
                    <div class="shared-content"><?php echo e($sp->content); ?></div>
                    <?php endif; ?>
                </div>
                <?php endif; ?>

                <!-- Post Media -->
                <?php if($post->media && $post->media->count()): ?>
                    <?php
                    ...
                    ?>
                    <?php if($imgCount == 1): ?>
                        <img src="<?php echo e(asset('storage/'.$images->first()->path)); ?>" class="post-media-single">
                    <?php elseif($imgCount == 2): ?>
                        <div class="post-media-grid cols-2">
                            ...
                            <div class="media-item"><img src="<?php echo e(asset('storage/'.$im->path)); ?>"></div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    <?php elseif($imgCount >= 3): ?>
                        <div class="post-media-grid cols-3">
                            <?php $__currentLoopData = $images->take(5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $im): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="media-item">
                                <img src="<?php echo e(asset('storage/'.$im->path)); ?>">
                                <?php if($i == 4 && $imgCount > 5): ?>
                                <div class="media-more-overlay">+<?php echo e($imgCount - 5); ?></div>
                                <?php endif; ?>
                            </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    ...
                    <?php $__currentLoopData = $videos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <video controls class="post-media-single"><source src="<?php echo e(asset('storage/'.$v->path)); ?>"></video>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php endif; ?>
                ...
                <div class="post-stats">
                    ...
                    ?>
                    <div class="stats-reactions <?php echo e($post->likes_count > 0 ? '' : 'hidden'); ?>" id="likes-wrap-<?php echo e($post->id); ?>">
                        <div class="reaction-icons">
                            <?php $__currentLoopData = $topTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <span class="reaction-icon <?php echo e($t); ?>"><i class="<?php echo e($statIconClassMap[$t]); ?>"></i></span>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php if($topTypes->isEmpty()): ?>
                            <span class="reaction-icon like"><i class="far fa-thumbs-up"></i></span>
                            <?php endif; ?>
                        </div>
                        <span class="stats-count" data-reactions-modal="<?php echo e($post->id); ?>" id="likes-count-<?php echo e($post->id); ?>"><?php echo e($post->likes_count); ?></span>
                    </div>
                    <div class="stats-right">
                        <span class="stats-count" data-toggle-comments="<?php echo e($post->id); ?>">
                            <span id="comments-count-<?php echo e($post->id); ?>"><?php echo e($post->comments_count); ?></span> bình luận
                        </span>
                        <span class="stats-count">
                            <span id="shares-count-<?php echo e($post->id); ?>"><?php echo e($post->shares_count ?? 0); ?></span> chia sẻ
                        </span>
                    </div>
                </div>
                ...
                <div class="post-actions">
                    ...
                    <div class="like-wrapper">
                        <button class="action-btn like-btn <?php echo e($hasReacted ? 'reacted-'.$reactionType : ''); ?>"
                                data-react-endpoint="<?php echo e(route('posts.react',$post)); ?>"
                                data-post-id="<?php echo e($post->id); ?>"
                                data-reaction="<?php echo e($reactionType ?? ''); ?>">
                            <i class="<?php echo e($reactionIconMap[$reactionType] ?? 'far fa-thumbs-up'); ?>"></i>
                            <span><?php echo e($reactionTextMap[$reactionType] ?? 'Thích'); ?></span>
                        </button>
                        <div class="reactions-popover" data-for-post="<?php echo e($post->id); ?>">
                    ...
                    <button class="action-btn" data-toggle-comments="<?php echo e($post->id); ?>">
                        <i class="far fa-comment"></i>
                        <span>Bình luận</span>
                    </button>

                    <form method="POST" action="<?php echo e(route('posts.store')); ?>" style="margin:0">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="shared_post_id" value="<?php echo e($post->id); ?>">
                ...
                <div class="comments-section" id="comments-<?php echo e($post->id); ?>">
                    <form method="POST" action="<?php echo e(route('posts.comment',$post)); ?>" class="comment-form">
                        <?php echo csrf_field(); ?>
                        <input name="content" class="comment-input" placeholder="<?php echo e(__('app.feed.write_comment')); ?>">
                        <button type="submit" class="comment-submit">Gửi</button>
                    </form>
                    
                    <?php $__currentLoopData = $post->comments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="comment-item">
                        <div class="comment-avatar"></div>
                        <div style="flex:1">
                            <div class="comment-bubble">
                                <div class="comment-author"><?php echo e($c->user->name ?? 'User'); ?></div>
                                <div class="comment-text"><?php echo e($c->content); ?></div>
                            </div>
                            <div class="comment-actions">
                                <span class="comment-time"><?php echo e($c->created_at->diffForHumans()); ?></span>
                                <?php $myCReaction = auth()->check() ? optional($c->reactions->firstWhere('user_id', auth()->id()))->type : null; ?>
                                <span class="comment-action <?php echo e($myCReaction ? 'active' : ''); ?>"
                                      data-react-endpoint="<?php echo e(route('comments.react',$c)); ?>"
                                      data-comment-id="<?php echo e($c->id); ?>"
                                      data-reaction="<?php echo e($myCReaction ?? ''); ?>">
                                    <?php echo e($myCReaction ? (['like'=>'Thích','love'=>'Yêu thích','haha'=>'Haha','wow'=>'Wow','sad'=>'Buồn','angry'=>'Phẫn nộ'][$myCReaction] ?? 'Thích') : 'Thích'); ?>
...
<div class="modal-overlay" id="composerModal">
    <div class="modal-content" onclick="event.stopPropagation()">
        <div class="modal-header">
            <h3 class="modal-title"><?php echo e(__('app.feed.create_post')); ?></h3>
        ...
        <form method="POST" action="<?php echo e(route('posts.store')); ?>" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                <textarea class="composer-textarea" name="content" rows="4" placeholder="<?php echo e(__('app.feed.whats_on_your_mind')); ?>"></textarea>
...
<?php $__currentLoopData = $posts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $post): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
<?php $canEdit = auth()->check() && ((auth()->id() === $post->user_id) || in_array(auth()->user()->user_role,['admin','super_admin'])); ?>
<?php if($canEdit): ?>
<div class="modal-overlay" id="editModal<?php echo e($post->id); ?>">
    <div class="modal-content" onclick="event.stopPropagation()">
        <div class="modal-header">
            <h3 class="modal-title">Chỉnh sửa bài viết</h3>
            <button class="modal-close" onclick="closeEditModal(<?php echo e($post->id); ?>)"><i class="fas fa-times"></i></button>
        </div>
        <form method="POST" action="<?php echo e(route('posts.update',$post)); ?>">
            <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
            <div class="modal-body">
                <textarea class="composer-textarea" name="content" rows="5"><?php echo e($post->content); ?></textarea>
            </div>
            <div class="modal-footer">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\MarketPlace\resources\views/posts/index.blade.php ENDPATH**/ ?>