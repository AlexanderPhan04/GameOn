    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="admins-hero">
            <div class="hero-content flex items-center justify-between flex-wrap gap-4">
                ...
                <div class="hero-buttons flex gap-3">
                    <a href="{{ route('admin.admins.create') }}" class="btn-neon btn-neon-success">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div>
                    <div class="stat-value">{{ $superAdminCount ?? 1 }}</div>
            ...
            <div class="stat-card">
                ...
                <div>
                    <div class="stat-value">{{ $pendingInvitations->count() }}</div>
            ...
            <div class="stat-card">
                ...
                <div>
                    <div class="stat-value">{{ count(\App\Models\AdminPermission::AVAILABLE_PERMISSIONS) }}</div>
        ...
        <div class="table-card">
            <div class="table-header">
                ...
                <span class="table-count">{{ $admins->count() }} admin</span>
            </div>
                            ...
                            <tr>
                                <td>
                                    <div class="flex items-center gap-3">
                                        <img src="{{ get_avatar_url($admin->avatar) }}" 
                                        ...
                                        @elseif(count($permissions) > 0)
                                            @foreach($permissions as $perm)
                                                @if(isset($availablePermissions[$perm]))
                                                    <span class="permission-badge" title="{{ $availablePermissions[$perm]['description'] }}">
                                                        <i class="{{ $availablePermissions[$perm]['icon'] }}"></i>
                                                        {{ Str::limit($availablePermissions[$perm]['label'], 15) }}
                                                    </span>
                                                @endif
                                            @endforeach
                                        @else
                                ...
                                <td>
                                    @if($admin->adminPermission)
                                        <span style="color: #94a3b8;">{{ $admin->adminPermission->created_at->format('d/m/Y') }}</span>
                                    @else
        ...
        <div class="table-card">
            <div class="table-header">
                ...
                <span class="table-count">{{ $pendingInvitations->count() }} lời mời</span>
            </div>
                    ...
                    @foreach($pendingInvitations as $invitation)
                        <div class="invitation-card">
                            <div class="invitation-info">
                                ...
                                <div>
                                    ...
                                    <div class="invitation-meta">
                                        <span><i class="fas fa-clock"></i> {{ $invitation->created_at->diffForHumans() }}</span>
                                        <span><i class="fas fa-hourglass-half"></i> Hết hạn: {{ $invitation->expires_at->format('d/m/Y H:i') }}</span>
                            ...
                            <div class="invitation-actions">
                                @if(!$invitation->isExpired())
                                    <form action="{{ route('admin.admins.resend-invitation', $invitation) }}" method="POST" style="display: inline;">
                                        @csrf
                                ...
                                @endif
                                <form action="{{ route('admin.admins.cancel-invitation', $invitation) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                ...
                @foreach($availablePermissions as $key => $permission)
                    <div class="perm-item">
                        ...
                        <div>
                            ...
                            <div class="perm-desc">{{ Str::limit($permission['description'], 40) }}</div>
                        </div>
                    </div>
                @endforeach
            </div>
