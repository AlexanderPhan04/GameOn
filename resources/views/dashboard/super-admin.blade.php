    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="welcome-card mb-6">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 relative z-10">
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        <h1 class="welcome-title">{{ __('app.dashboard.welcome') }}, <span>{{ Auth::user()->display_name }}!</span></h1>
                        <p class="text-[#94a3b8] text-base">{{ __('app.dashboard.admin_dashboard') }} • {{ __('app.dashboard.manage_users_teams_esports') }}</p>
                    </div>
                </div>
                <div class="time-widget">
                    <div class="time-display" id="current-time">{{ now()->timezone('Asia/Ho_Chi_Minh')->format('H:i:s') }}</div>
                    <p class="text-[#94a3b8] text-sm mt-1">{{ now()->timezone('Asia/Ho_Chi_Minh')->format('d/m/Y') }}</p>
                    <p class="text-[#00E5FF] text-sm font-semibold">{{ now()->timezone('Asia/Ho_Chi_Minh')->locale(app()->getLocale())->dayName }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="stats-grid mb-6">
            ...
            <div class="stat-card">
                ...
                <p class="stat-label"><i class="fas fa-users text-xs"></i> {{ __('app.dashboard.total_users') }}</p>
                <h2 class="stat-number">{{ number_format($stats['total_users']) }}</h2>
                <div class="mt-4 pt-3 border-t border-[rgba(0,229,255,0.1)]">
                    <a href="{{ route('admin.users.index') }}" class="stat-link">{{ __('app.dashboard.view_details') }} <i class="fas fa-arrow-right text-xs"></i></a>
                </div>
            </div>
            ...
            <div class="stat-card">
                ...
                <div class="mt-4 pt-3 border-t border-[rgba(0,229,255,0.1)]">
                    <a href="{{ route('admin.users.index') }}?role=admin" class="stat-link">{{ __('app.dashboard.manage_users') }} <i class="fas fa-arrow-right text-xs"></i></a>
                </div>
            </div>
            ...
            <div class="p-5">
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                    <a href="{{ route('admin.users.index') }}" class="quick-action">
                        ...
                        <p class="quick-action-text">{{ __('app.dashboard.manage_users') }}</p>
                    </a>
                    <a href="{{ route('admin.teams.index') }}" class="quick-action">
                        <div class="quick-action-icon bg-gradient-to-br from-[#10b981] to-[#059669]"><i class="fas fa-users-cog text-white"></i></div>
                        <p class="quick-action-text">{{ __('app.profile.manage_teams') }}</p>
                    </a>
                    <a href="{{ route('admin.tournaments.index') }}" class="quick-action">
                        <div class="quick-action-icon bg-gradient-to-br from-[#6366f1] to-[#4f46e5]"><i class="fas fa-trophy text-white"></i></div>
                        <p class="quick-action-text">{{ __('app.profile.manage_tournaments') }}</p>
                    </a>
                    <a href="{{ route('admin.games.index') }}" class="quick-action">
                        <div class="quick-action-icon bg-gradient-to-br from-[#f59e0b] to-[#d97706]"><i class="fas fa-gamepad text-white"></i></div>
                        <p class="quick-action-text">{{ __('app.profile.manage_games') }}</p>
                    </a>
                    <a href="{{ route('posts.index') }}" class="quick-action">
                        <div class="quick-action-icon bg-gradient-to-br from-[#ec4899] to-[#db2777]"><i class="fas fa-newspaper text-white"></i></div>
                        <p class="quick-action-text">{{ __('app.nav.posts') }}</p>
                    </a>
                    <a href="{{ route('chat.index') }}" class="quick-action">
                        ...
                        <p class="quick-action-text">{{ __('app.nav.chat') }}</p>
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-6">
            ...
            <div class="modern-card">
                <div class="modern-card-header">
                    <h3 class="modern-card-title"><i class="fas fa-chart-pie"></i> {{ __('app.dashboard.activity_statistics') }}</h3>
                </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between mb-1">
                                    <span class="text-white font-semibold text-sm">{{ ucfirst(str_replace('_', ' ', $role->user_role)) }}</span>
                                    <span class="text-[#00E5FF] font-bold">{{ $role->count }}</span>
                                </div>
                                <div class="progress-track"><div class="progress-fill" style="width: {{ ($role->count / max($stats['total_users'], 1)) * 100 }}%"></div></div>
                            </div>
                        </div>
                    </div>
                    @endif
        ...
        <div class="modern-card">
            <div class="modern-card-header">
                <h3 class="modern-card-title"><i class="fas fa-users"></i> {{ __('app.dashboard.recent_users') }}</h3>
                <div class="flex gap-3">
                    <button class="btn-ghost" onclick="location.reload()"><i class="fas fa-sync-alt mr-1"></i> {{ __('app.common.refresh') }}</button>
                    <a href="{{ route('admin.users.index') }}" class="btn-neon"><i class="fas fa-arrow-right"></i> {{ __('app.dashboard.view_all') }}</a>
                </div>
            </div>
            <div class="p-5">
    ...
    if (ctx) {
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{ label: 'Users', data: [5, 8, 12, 18, 25, 32, 38, 45, 52, 60, 70, {{ $stats['total_users'] }}], borderColor: '#00E5FF', backgroundColor: 'rgba(0, 229, 255, 0.1)', fill: true, tension: 0.4, borderWidth: 3, pointBackgroundColor: '#00E5FF', pointBorderColor: '#000814', pointBorderWidth: 2, pointRadius: 4 }]
            },
